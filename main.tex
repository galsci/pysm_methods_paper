...
The first CO model {\bf\texttt{co1}} adopts the $J = 1\rightarrow0$ template as-is and assumes no polarization. Emission from the other two transitions is computed assuming constant rescaling factors $0.595$ and $0.297$ for $J = 2\rightarrow1$ and $J = 3\rightarrow2$ \citep[Sect. 4.2.3]{planck2013-p03a}, obtained  from a dedicated  analysis on small bright CO regions (Taurus, Orion, Polaris). We do not employ the Planck maps of emission in these transitions directly since their signal-to-noise ratio is much lower.
