...
%In the first panel, I show PySM 3.4.0 d9/d10/d12 Q/U maps integrated with delta bandpass 85/150/220/270/353 GHz. (Although 353 GHz may not be an observation channel of S4, I include this frequency as well to allow comparison with previous analyses.) These maps are simply limited to the non-zero B3 observation region to draw a contrast with other panels — the apodization mask is not involved yet.

%The second panel shows the PySM input maps convolved with the B3 beam. One can see the beam-smoothing effect clearly. This is a necessary step before applying the observation matrix.

%The third panel shows the results after further multiplying the BK18 B3 observation matrix to the smoothed maps. These maps are now suppressed by the beam-smoothing, timestream processing (mainly the third-order polynomial filtering along RA) and beam deprojection. As a result, these maps lose their large-scale structures and their color scale is decreased by 10 times. It seems like the EE/BB ratio of these maps are just slightly larger than 1 in the B3 sky patch, so one can't see an obvious "plus-like" or "cross-like" pattern in the Q/U maps.

%Finally, in the fourth panel the maps are multiplied by the B3 purification matrix and masked by the inverse noise variance apodization mask. This operation decreases the map effective area to 585 square degrees, and now one can see a "cross-like" pattern in Q and "plus-like" pattern in U under a one time smaller color scale, meaning that the pure B-modes are separated out. I show the masked version here to demonstrate that the edge signals are entirely suppressed, ensuring that we will not run into the issues shown in the "Pole deep" mask (see posting 20220425_fil_model). I also include the E-mode purified versions to make sure that the reverse Q/U patterns can be seen under the same color scale.
