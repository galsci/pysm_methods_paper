...
To construct the modulation maps $m_i$ and $m_p$, we generally follow the method outlined in \citet{Thorne:2017}. For each pixel at the location $\hat{\theta}$ in an $N_{\rm side} = 8$ HEALPix map, we define a circular mask centered on $\hat{\theta}$ of radius $11.3^\circ$  and apodize it with a $2^\circ$ Gaussian profile. We then compute the $tt$ and $ee$ power spectra within that mask via the NaMaster estimator \citep{Alonso:2019}. The modulation maps are constructed from the ratios 
...
As described in Section~\ref{subsubsec:synch_model}, the synchrotron emission in all models developed here is governed by the spectral parameters $\beta_s$ and $c_s$. To build the large scale template for the spatial variation of the synchrotron spectral index $\beta_s$, we begin with the full-sky $\beta_s$ map of \citet{Miville-Deschenes:2008} obtained by combining the Haslam map in total intensity at 408\,MHz \citep{Remazeilles:2015} and WMAP K-band data. The map has an angular resolution of about $7^{\circ}$ and was employed by previous PySM synchrotron models \citep{Thorne:2017}.
...
For the synchrotron curvature parameter $c_s$, there are no readily available template maps. The existing \texttt{s3} model implements curvature as a sky-constant $c_s = -0.052$, consistent with the measurements from ARCADE \citep[$c_s=-0.052 \pm 0.005$,][]{Kogut:2012}. As an attempt to model a reasonable range of spatial variability, we assume that fluctuations in the value of $c_s$ follow the synchrotron intensity at large angular scales. Specifically, we start from the Haslam map 408\,MHz $I$ map \citep{Remazeilles:2015} smoothed to a resolution of $5 \deg$, which we then rescale with a linear transformation such that the minimum and maximum pixel values over the full sky are $-0.0517$ and $0.0054$, respectively, corresponding to the range of $c_s$ measurements from ARCADE \citep{Kogut:2012}. The resulting $c_s$ map has a mean and standard deviation of $-0.0517$ and $0.0054$, respectively, within the ARCADE footprint.

To extend our $c_s$ map to angular scales $\ell > \ell_1 = 36$, we first generate a map of Gaussian random fluctuations with the same power law index as the $\beta_s$ map, i.e., $\alpha _{c_s}=\alpha _{\beta_s} = -0.61$. We modulate the resulting map with the same modulation map as used for $\beta_s$ and then combine this high-$\ell$ map with the low-$\ell$ template following the filter function of Equation~\ref{eq:filter}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.48\textwidth]{figures/llmean_hlmean_hist.pdf}
    \caption{...}
    \label{fig:smallfield_ratio_hist}
\end{figure}

%Pink dashed reference lines representing $\mathcal{D}_\ell^{BB} \propto \ell^{-0.42}$ are added to the bottom panel. This power law, derived from the analyses of larger sky regions ($f_\text{sky} = 0.3 \; \text{to} \; 0.8$) by \cite{planck2014-XXX}, also fits the small-field NPIPE data points, which spread out when the dust amplitude is small presumably due to map noise. Additionally, the trends for \texttt{d1}, \texttt{d9} and \texttt{d12} data points are slightly lower than those for NPIPE in these log-log plots, suggesting that the model spectra are steeper.

To explore whether the PySM models can reproduce the observed dust $BB$ power spectrum's amplitude and scaling-in-$\ell$ especially in those high Galactic latitude small fields which could be part of future CMB observations, we compute the low-$\ell$ averaged band power $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ (over $40 \le \ell < 110$) and the high-$\ell$ averaged band power $\overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$ (over $110 \le \ell < 370$) of each small field for the model and NPIPE maps. These two quantities therefore reduce the impact of noise variation in the NPIPE $BB$ data and can be taken as proxies of dust amplitude for the model against data comparison. We present the results as scatter plots with the model's quantities plotted on the y-axis and the NPIPE's quantities plotted on the x-axis in Figure~\ref{fig:smallfield_power_all}.

While both \texttt{d1} and \texttt{d9} have power amplitudes that are generally consistent with the NPIPE amplitude, the \texttt{d9} model shows a major advance in the correlation, increasing the correlation coefficient from 0.887 to 0.998 in the low-$\ell$ case. This feature is especially obvious in the regimes where the signal-to-noise ratio is high, for example in the $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ comparison or in the small fields where the dust amplitude is large. However, when $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ and $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ are smaller than $10 \; \mu\rm{K_{CMB}}^2$, the \texttt{d9} model systematically overestimates the dust amplitude. In contrast, the $\texttt{d12}$ model systematically underestimates the dust amplitude in all cases. 
...
%We define the ratio of the low-$\ell$ mean value (plotted on the x-axis) to the high-$\ell$ mean value (plotted on the y-axis) as a proxy for describing the spatial power change across the modulation scale.

%On the scaling-in-$\ell$, the perfect fixed-index power law $\mathcal{D}_\ell^{BB} \propto \ell^{-0.42}$ yields a value of $\sim 1.60$ for this ratio, while the NPIPE maps give $1.86 \pm 0.93$ and the \texttt{d1}, \texttt{d9} and \texttt{d12} model maps give $2.02 \pm 0.69$, $2.37 \pm 0.77$ and $2.24 \pm 0.87$ respectively over sky patches with $|b| > 30^\circ$.
% talk about several things - amplitude, distribution and index...split right after figure 7
% amplitude~low-ell average: quantify the fraction > low-ell 10^2, answer Brandon's question 
