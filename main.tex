...
\begin{deluxetable*}{m{0.5cm} m{2.8cm} m{2.5cm} m{2.2cm} m{2.4cm} m{2.2cm} m{2.6cm}} \label{table:summarydust}
\caption{Summary of the PySM 3.4 models --- Dust}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Model} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} \\ & & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{d9} & Modified blackbody & GNILC PR2 ($I$) + GNILC PR3 ($Q$/$U$) 353 GHz & Modulated + polarization fraction tensor & \centering Uniform spectral parameters & \centering ---  & \centering Deterministic \tabularnewline
\hline
\\
...
\hline
\texttt{d11} & \centering Same as above & \centering Same as above & Same as above & Same as above & \centering Same as above & Stochastic $I$/$Q$/$U$ \& spectral parameters \\
\hline
\texttt{d12} & 6 layers, each modelled as a different modified blackbody &  GNILC PR2 ($I$) + PR3 ($Q$/$U$) 353 GHz & Modulated + gaussian & \centering Random realization for each layer & \centering  Random realization for each layer & \centering Deterministic \tabularnewline
%\bottomrule
%\end{tabular}
%\end{center}
\enddata
...
\begin{deluxetable*}{m{0.5cm} m{2.8cm} m{2.5cm} m{2.2cm} m{2.4cm} m{2.2cm} m{2.6cm}} \label{table:summarysynch}
\caption{Summary of the PySM 3.4 models --- Synchrotron}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Model} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} \\ & & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{s4} & \centering Power law  & Haslam ($I$) 408 MHz + WMAP ($Q$/$U$) 23 GHz & Modulated + polarization fraction tensor & \centering Uniform spectral index & \centering --- & \centering Deterministic \tabularnewline
\hline
\\
...
\hline
\\
\texttt{s6} & \centering Power law & \centering Same as above & Same as above & Same as above & \centering Same as above & Stochastic $I$/$Q$/$U$ \& spectral index \tabularnewline
\hline
\\
...
\begin{deluxetable*}{m{0.5cm} m{3cm} m{4.2cm} m{4.4cm} m{2.2cm}} \label{table:summaryco}
\caption{Summary of the PySM 3.4 models --- CO}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Model} & \colhead{Templates} & \colhead{Templates} & \colhead{Stochasticity} \\ & & \colhead{at large scales} & \colhead{at small scales}}
\startdata
\texttt{co1} & Single line emissions at 115, 230, 346~GHz & \centering PR1 MILCA smoothed to $1^{\circ}$ & \centering --- & \centering Deterministic \tabularnewline
\hline
\texttt{co2} & Same as above + $0.1\%$ polarized & \centering Same as above & \centering ---  & \centering Deterministic \tabularnewline
\hline
\\
