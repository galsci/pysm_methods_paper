...
Galactic CO line emission is strong enough that the $J = 1\rightarrow0$, $J = 2\rightarrow1$, $J = 3\rightarrow2$ transitions have been detected even in the broad photometric Planck bands \citep{planck2013-p03a, planck2014-a12}. Since this emission can be polarized \citep{Goldreich:1981}, CO emission could be a relevant foreground for CMB polarization analyses \citep{Puglisi:2017}. Thus, we provide an implementation of three CO emission and polarization models developed by \citet{Puglisi:2017} into the PySM framework.

All CO models implemented here are based on the CO $J = 1\rightarrow0$ \texttt{Type-1} intensity map\footnote{\url{HFI_CompMap_CO-Type1_2048_R2.00.fits}} derived by \citet{planck2013-p03a}. This CO map is obtained exploiting the mismatches in the detector bandpass to recover the CO from the CMB and the other Galactic foregrounds, with the MILCA component separation algorithm. This template is preferred to those from spectroscopic surveys \citep[e.g.,][]{Dame:2001} because it is not limited to low Galactic latitudes and has a uniform angular resolution. Although the map is released at a resolution of 10\arcmin, we convolve it with a 1$^\circ$ Gaussian beam to reduce noise contamination especially at intermediate and high Galactic latitudes. The template is then downgraded to coarser pixellization of \texttt{nside=512}.

The first CO model {\bf\texttt{co1}} encodes   intensity -only  emission from the Planck templates once preprocessed with the steps listed above. Templates  for the other two lines, namely  $J = 2\rightarrow1$ and $J = 3\rightarrow2$,  are derived  assuming two constant rescaling factors $0.595$ and $0.297$ as in  \citep[Sect. 4.2.3]{planck2013-p03a}. This is mainly because the templates for the other lines  employ different signal-to-noise ratio as the CO emission gets dimmer at higher transitions.  

The complexity in the other two CO  models  gradually increases, and it aims at modelling polarized CO emission both at low and high Galactic latitudes.  The {\bf\texttt{co2}} model  accounts also  for  polarized emission,  with fractional polarization left as a free parameter that can be set by the user (default $p = 0.1 \%$). For this model, Stokes $Q$ and $U$ CO maps are modeled by considering  polarization templates from the  COMMANDER thermal dust maps \citep{planck2014-a12}   \citet[Sect. 4]{Puglisi:2017}.  

Finally, the {\bf\texttt{co3}} not only accounts for the polarized emission as in {\bf\texttt{co2}} but it also includes     small scale emission from high-Galactic latitude clouds with the procedure outlined in \citet{Puglisi:2017}.  We specifically consider the {\bf\texttt{LogSpiral}} model from  \citet{Puglisi:2017} that best fitted the Planck data and perform a dedicated simulation to produce the template   of  polarized and unpolarized  high-latitude Galactic clouds. This $IQU $ template is finally  coadded with the  {\bf\texttt{co2}} one.
