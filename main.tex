...
However, in the fields where $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ and $\overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$ are smaller than $10 \; \mu\rm{K_{CMB}}^2$, \texttt{d9} systematically overestimates the dust amplitude. Conversely, $\texttt{d12}$ underestimates the dust amplitude in 94\% (85\%) of the small fields for low-$\ell$ (high-$\ell$). These trends are also observed in other practical small fields, for example the BICEP/Keck field that we will discuss in Section~\ref{sec:BK_validation}, and the independent analysis of the SPIDER field by \cite{SPIDERCollaboration:2024} (although the latter results were presented at 150~GHz).
