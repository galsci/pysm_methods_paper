...
The complexity in the other two CO  models  gradually increases, and it aims at modelling polarized CO emission both at low and high Galactic latitudes.  The {\bf\texttt{co2}} model  accounts also  for  polarized emission,  with fractional polarization left as a free parameter that can be set by the user (default $\Pi = 0.1 \%$). For this model, Stokes $Q$ and $U$ CO maps are modeled by considering  polarization templates from the  COMMANDER thermal dust maps \citep{planck2014-a12}   \citet[Sect. 4]{Puglisi:2017}.  

Finally, the {\bf\texttt{co3}} not only accounts for the polarized emission as in {\bf\texttt{co2}} but it also includes     small scale emission from high-Galactic latitude clouds with the procedure outlined in \citet{Puglisi:2017}.  We specifically consider the {\bf\texttt{LogSpiral}} model from  \citet{Puglisi:2017} that best fitted the \emph{Planck } data and perform a dedicated simulation to produce the template   of  polarized and unpolarized  high-latitude Galactic clouds. This $IQU $ template is finally  coadded with the  {\bf\texttt{co2}} one.
