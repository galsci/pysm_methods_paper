...
The first CO model {\bf\texttt{co1}} adopts the $J = 1\rightarrow0$ template as-is and assumes no polarization. Emission from the other two transitions is computed directly from the $J = 1\rightarrow0$ template with constant rescaling factors $0.595$ and $0.297$ for $J = 2\rightarrow1$ and $J = 3\rightarrow2$, respectively, consistent with observations of small CO-bright regions like Taurus, Orion, and Polaris \citep[][Section~4.2.3]{planck2013-p03a}. We do not employ the Planck maps of emission in these transitions directly since their signal-to-noise ratio is much lower.
