...
This prescription has several free parameters that require tuning. The pivot scale $\ell_1$ governs up to which multipole information is taken from the input template maps versus what is generated randomly. The scale $\ell_0$ avoids using the largest scales in the power spectrum fit as they are subject to large sample variance. %often not well-described by a power law.
