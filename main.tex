...
\begin{align}\label{eq:real2pt}
    ...
    ~~~,
\end{align}
where $P \equiv \sqrt{Q^2 + U^2}$. Through the normalization of $Q$ and $U$ by $P$, maps of $q$ and $u$ are designed to have fluctuations roughly independent of proximity to the Galactic plane. % BH: I have not fully captured the physical/mathematical raison d'être here, probably need to coordinate with Andrei. E.g., why this functional form?
...
\begin{table*}
    ...
    High  & \texttt{d12, s7, f1, a2, co3} & MKD dust layer model, spatially varying synchrotron curvature, polarized AME, CO.  \\
...
The high complexity model set models Galactic emission properties that are physically realistic but as-yet undetected, like polarized AME and spatially varying synchrotron curvature. The high complexity model set uses the MKD layer model for Galactic dust emission, and the decorrelation at frequencies dominated by the dust emission is near the maximum allowed by current constraints (Figure \ref{fig:decorrelation}).
