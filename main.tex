...
\item To obtain only the small scales, we apply a high-pass filter to the fitted power law power spectra with $\ell_{cut} = 200$ for intensity and polarization maps, and then generate full sky small scales realizations of this filtered power law spectra (with $\ell_{max}=4096$) with the \texttt{synfast} routine provided by HEALPix. 
...
For the second set of maps, we need to mimic the effective modulation of \texttt{d10} maps in $IQU$ quantities. We use the following procedure:
\begin{enumerate}
