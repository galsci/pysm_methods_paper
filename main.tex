...
We organize the paper as follows: Section~\ref{sec:methods} is an overview of how Galactic emission models are implemented in PySM; Section~\ref{sec:small_scales} presents our methodology for generating stochastic simulations of small-scale emission using the polarization fraction tensor formalism; Section~\ref{sec:other_models} describes our implementation of an alternative dust model and of a suite of CO emission models; Section~\ref{sec:validation} details a collection of validation metrics for the new models; Section~\ref{sec:discussion} discusses current limitations and future directions for development; Section \ref{sec:modelsuite} presents our proposed triplet of sky models that span a range of complexity; and Section~\ref{sec:summary} concludes with a summary.

\section{Methods Overview} \label{sec:methods}

\subsection{The PySM Software}
The PySM software provides a convenient interface for generating full-sky maps of total and polarized emission at far-infrared through radio frequencies. Users can select one or more emission mechanisms to simulate, including the CMB, dust, synchrotron radiation, free-free emission, and CO line emission. Most emission mechanisms have more than one model to select from, where here a model refers both to the spatial morphology and frequency dependence of the emission. Stokes $I$, $Q$, and $U$ maps are provided in HEALPix\footnote{https://healpix.sourceforge.io}  \citep{Gorski:2005} format at the requested $N_{\rm side}$ at a user-specified frequency or integrated over a user-specified bandpass.
...
\begin{deluxetable*}{   m{0.5cm} m{2cm}  m{2.2cm}  m{2cm} m{2.2cm}  m{2cm}  m{2cm}  m{.7cm}  } 
...
\begin{deluxetable*}{   m{0.5cm} m{2cm}  m{2.2cm}     m{2cm}  m{2cm}  m{.7cm}  } 
...
To implement the parametric models of dust and synchrotron emission described in Section~\ref{sec:emission_mechanisms}, we require maps of $A^I$, $A^Q$, and $A^U$ for each mechanism, i.e., the total emission and polarization at a reference frequency. We start from template $I$, $Q$, and $U$ maps derived from data, to which we add fluctuations at scales that are noise-dominated. Our treatment of amplitude parameters differs from spectral parameters (Section~\ref{sec:spec_params}) because amplitude parameters need all of $I$, $Q$, and $U$ and thus consistency among them.
...
\begin{align}\label{eq:pt2real}
    ...
    ~~~,
\end{align}
where $\xi \equiv \sqrt{q^2 + u^2}$. A number of useful features of this transformation are evident. First, for any values of $i$, $q$, and $u$,  $I$ is guaranteed to be positive. Further, it is straightforward to show that

\begin{equation}
    p = \tanh\xi
    ~~~,
\end{equation}
and so $0 \leq p \leq 1$. 
Since the transformation is non-linear, Gaussian fluctuations introduced in $i$, $q$, and $u$ maps will necessarily become non-Gaussian when transformed back to $I$, $Q$, and $U$.
...
Thus, the method for generating small-scale fluctuations \af{in polarized emission} is summarized as follows: 
\begin{enumerate}
...
% we should start the section with how all the CO models share an I map, and it is made in this way. they differ in polarization. co1 has no polarization, while co2 and co3 follow different recipes from Puglisi. here is a summary.
The PySM3 employs models of CO emission based on   the CO $J = 1\rightarrow0$ \texttt{Type-1} intensity map\footnote{\url{HFI_CompMap_CO-Type1_2048_R2.00.fits}} released by \citet{planck2013-p03a}.  This CO map is obtained exploiting the mismatches in the detector bandpass to recover the CO from the CMB and the other Galactic foregrounds, with the MILCA component separation algorithm.  This template is therefore shared among all the pysm models described here. We employ this map as a template  instead of the one from 
\citet{Dame:2001}, because it is not limited to low Galactic latitudes and also it has more uniform resolution than the \citet{Dame:2001} one. 

However, although the map is released at the nominal resolution of  10\arcmin, we convolve it with a 1$^\circ$ gaussian beam to further lower the noise contamination  especially at intermediate and high Galactic latitudes. The template is then downgraded to coarser resolution of \texttt{nside=512}.  

The first CO model {\bf\texttt{co1}} encodes   intensity -only  emission from the \emph{Planck } templates once preprocessed with the steps listed above. Templates  for the other two lines, namely  $J = 2\rightarrow1$ and $J = 3\rightarrow2$,  are derived  assuming two constant rescaling factors $0.595$ and $0.297$ as in  \citep[sect. 4.2.3]{planck2013-p03a}. This is mainly because the templates for the other lines  employ different signal-to-noise ratio as the CO emission gets dimmer at higher transitions.  

The complexity in the other two CO  models  gradually increases, and it aims at tackling the lack of polarized CO emission both at low and high Galactic latitudes.  The {\bf\texttt{co2}} model  accounts also  for  polarized emission,  with fractional polarization left as a free parameter that can be set by the user (default $\Pi = 0.1 \%$). For this model, Stokes $Q$ and $U$ CO maps are modeled by considering  polarization templates from the  COMMANDER thermal dust maps \citep{planck2014-a12}   \citet[eqs. (12) and (13)]{Puglisi:2017}.  

Finally, the {\bf\texttt{co3}} not only accounts for the polarized emission as in {\bf\texttt{co2}} but it also includes     small scale emission from high-Galactic latitude clouds with the procedure outlined in \citet{Puglisi:2017}.  We specifically consider the \emph{LogSpiral} model from  \citet{Puglisi:2017} that best fitted the \emph{Planck } data and perform a dedicated simulation to produce the template   of  polarized and unpolarized  high-latitude Galactic clouds. This $IQU $ template is finally  coadded with the  {\bf\texttt{co2}} one.
...
This work was supported by the National Science Foundation under grant No. AST-2106607 (PI S.E.C.). This work was carried out in part at the Jet Propulsion Laboratory, California Institute of Technology, under a contract with the National Aeronautics and Space Administration. We are grateful to the BICEP/Keck collaboration for providing their BK18 matrix products for the validation analysis of this paper. 
