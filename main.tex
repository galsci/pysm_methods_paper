...
\startdata
\texttt{d9} & GNILC PR2 ($I$) + GNILC PR3 ($Q$/$U$) 353 GHz & Modulated + polarization fraction tensor & \centering Uniform spectral parameters & \centering ---  & \centering Deterministic & Modified blackbody \\
\hline
\\
...
\colhead{Tag} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} & \colhead{Model} \\ & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{s4} & Haslam ($I$) 408 MHz + WMAP ($Q$/$U$) 23 GHz & Modulated + polarization fraction tensor & \centering Uniform spectral index & \centering --- & \centering Deterministic & \centering Power law \tabularnewline
\hline
\\
...
\begin{table}
   ...
   \bottomrule
    \end{tabular}
    \caption{Model parameters adopted for synthesizing small-scale dust and synchrotron emission, where $\mathcal{D}_\ell^{xy} \propto \ell^{\alpha_{xy}}$.}
