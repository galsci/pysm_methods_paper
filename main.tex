...
Although the dust $BB$ amplitudes in the \texttt{d9}, \texttt{d10} and \texttt{d12} models still differ from observations in the BK patch, resolving these discrepancies is beyond the scope of this study. Our goal is not to provide full sky simulations that guarantee consistency with all available full-sky and partial-sky observations. Power spectrum-based techniques for modeling require a degree of global averaging of spectrum parameters, which are known to vary across different regions of the sky \citep{planck2016-l04}. For instance, while dust amplitude can be modulated using large-scale templates, the spectral tilt $\alpha_{BB}$ has to be fixed to a single value for the entire sky, which is not realistic.
