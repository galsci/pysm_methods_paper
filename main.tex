...
\begin{table*}
   ...
   \bottomrule
    \end{tabular}
    \caption{Spectral indices adopted for synthesizing small scales, where $D_{\ell}^{XX} \propto \ell^{\alpha}$. The dust spectral index $\alpha_{tt}$ is taken from \citet{Miville-Deschenes:2016}, whereas the other dust indices are adopted from \citet{planck2016-l11A}. The synchrotron spectral indices are obtained from fits $D_{\ell} \propto \ell^{\alpha}$ in the range $\ell_0 < \ell< \ell_1$.}
