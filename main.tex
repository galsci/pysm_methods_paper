...
\begin{figure*}
    \centering
    \includegraphics[width=2.1\columnwidth]{figures/llmean_hlmean_stat.pdf}
    \caption{Scatter plots of the mean of the first two $BB$ band powers, $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$, and the mean of the last four $BB$ band powers, $\overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$, as illustrated in Figure~\ref{fig:smallfield_power}. The top panel compares $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ of NPIPE against \texttt{d1}, \texttt{d9}, \texttt{d12} and NPIPE itself. The middle panel shows the same comparison but for $\overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$. The bottom panel shows $\overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$ against $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ for each data set. Each data point represents the results from a circular sky patches with $|b| > 30^\circ$. As expected, all data points in the two upper-right subplots align with the black dashed reference lines indicating the 1:1 ratio, while displaying the distribution of NPIPE band powers over these sky patches. Pink dashed reference lines representing $\mathcal{D}_\ell^{BB} \propto \ell^{-0.42}$ are added to the bottom panel. This power law, derived from the analyses of larger sky regions ($f_\text{sky} = 0.3 \; \text{to} \; 0.8$) by \cite{planck2014-XXX}, also fits the small-field NPIPE data points, which spread out when the dust amplitude is small presumably due to map noise. Additionally, the trends for \texttt{d1}, \texttt{d9} and \texttt{d12} data points are slightly lower than those for NPIPE in these log-log plots, suggesting that the model spectra are steeper.}
    \label{fig:smallfield_power_all}
\end{figure*}

We explore whether the PySM models can reproduce the observed dust $BB$ power spectrum's amplitude, and scaling-in-$\ell$, especially in those high Galactic latitude small fields which could be part of future CMB observations. We therefore compute the low-$\ell$ averaged band power (over $40 \le \ell < 110$) and the high-$\ell$ averaged band power (over $110 \le \ell < 370$) of each small field for the model and NPIPE maps, and we present the results as scatter plots in Figure~\ref{fig:smallfield_power_all}. We define the ratio of the low-$\ell$ mean value (plotted on the x-axis) to the high-$\ell$ mean value (plotted on the y-axis) as a proxy for describing the spatial power change across the modulation scale.
