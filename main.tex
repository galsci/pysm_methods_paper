...
\begin{enumerate}
    ...
    \item The dust and synchrotron models implemented in this work are in better overall agreement with observational data relative to previous PySM models. In particular, the dust $BB$ amplitude and ell-dependence in the new models much more closely match the 353\,GHz Planck NPIPE maps (Figure~\ref{fig:smallfield_power_all}).
    \item CIB contamination
    \item We find that the polarization fraction tensor framework yields maps with appreciable non-Gaussian small-scale structure in small patches. However, we find that the non-Gaussianity, as measured by Minkowski functionals, greatly diminishes when averaging over large sky areas.
