...
To compare the performance of the latest PySM dust models \texttt{d9}, \texttt{d10} and \texttt{d12} with real data over a broad range of scales, we present their power spectra computed on both large-area and small-area masks at delta 353~GHz where the dust emissions dominate. We first integrate the PySM dust models with a 353~GHz delta bandpass and color-correct the NPIPE data maps (i.e. scale them by a factor of 0.92) to the same single frequency. The CMB dipole is further subtracted from the NPIPE temperature maps. We then apply the defined masks to the mean-subtracted PySM dust models and NPIPE ``A''/``B'' detector-split maps. We compute the cross-spectra of the masked NPIPE data splits to minimize noise correlation, bin the spectra with a dynamic $\ell$ interval, and scale the results into the full-sky $\mathcal{D}_\ell$ by the second moment of the mask. The same method is applied to calculate the auto-spectra of the masked model maps. 
...
In our small-area case, we mainly follow the method described in \cite{planck2014-XXX} to define the masks for spectra computation. We first use a HEALPix grid with $\texttt{nside} = 8$ to divide the full sky into 768 patches. At the center of each patch, we create a 400-square-degree circular mask with its edge tapered by a $2^\circ$ FWHM Gaussian, resulting in $f_{\rm sky} \approx 0.008$. 
