...
We find that the \texttt{d1} and \texttt{d12} models align well with the observed $TT$ spectrum, except that they underestimate the power spectrum at $\ell > 500$ when $f_{\rm sky} = 0.8$. \texttt{d9} shows similar behavior at $\ell \lesssim 100$ for all sky fractions. However, the power of this model declines more rapidly beyond that scale when $f_{\rm sky} = 0.6$ and $0.8$. It exhibits a power bump around $\ell \sim 100$ for $f_{\rm sky} = 0.2$.


as the parameters $c_1$, $c_2$ and $\gamma$ in Equation~

template extrapolation 
controlling the transition are optimized for polarization, 
