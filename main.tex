...
\hline
\texttt{d12} & 6 layers, each is a different modified blackbody & GNILC PR2 $I$ + PR3 $Q$/$U$ 353 GHz & Modulated + gaussian & \centering Random realization for each layer & \centering  Random realization for each layer & \centering Deterministic \tabularnewline
%\bottomrule
%\end{tabular}
%\end{center}
\enddata
...
Note that the color scale is different for the right panel of Fig.~\ref{fig:353_int}; it is mainly due to the uncertainty in the CIB monopole.\footnote{\textcolor{red}{For the generation of model {\tt d12}, a monopole of 0.09 MJy/sr is subtracted from the DR2 GNILC intensity map}} 
In the case of model {\tt d1} (b), it is due to the difference in the CIB monopole between PR3 and PR2 products, as model {\tt d1} is based on PR2 data. 
