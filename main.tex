...
\begin{deluxetable*}{m{0.5cm} m{2.5cm} m{2.2cm} m{2.2cm} m{2.2cm} m{2.5cm} m{2.2cm}} \label{table:summarydust}
...
\startdata
\texttt{d9} &  GNILC PR2 (I) + GNILC PR3 (QU) 353 GHz &  Modulated + polarization tensor & Uniform spectral parameters &  \centering ---  & Deterministic & Modified black body \\
\hline
\texttt{d10} & Same as above & Same as above &  $\beta_d, \, T_d$ from GNILC maps & Modulated & Deterministic & Same as above \\
\hline
\texttt{d11} & Same as above & Same as above & Same as above & Same as above & Stochastic $I$/$Q$/$U$ \& spectral parameters & Same as above \\
\hline
\texttt{d12} &  GNILC PR2 (I) + PR3 (QU) 353 GHz & Modulated + gaussian & Random realization for each layer & Random realization for each layer & Deterministic & 6 layers each modelled as a different modified black body \\
%\bottomrule
%\end{tabular}
%\end{center}
\enddata
...
\begin{deluxetable*}{m{0.5cm} m{2.5cm}  m{2.2cm}  m{2.2cm} m{2.2cm}  m{2.5cm}  m{2.2cm}} \label{table:summarysynch}
\caption{Summary of the PySM 3.4 models --- Synchrotron. All models have a maximum $N_\text{side} = 8192$.}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} & \colhead{Model} \\ & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{s4} & Haslam (I) 408 MHz + WMAP (QU) 23 GHz & Modulated + polarization tensor & Uniform spectral index & \centering --- & Deterministic & Power law \\
\hline
\texttt{s5} & Same as above & Same as above & $\beta_s$ from Haslam, S-PASS, WMAP & Modulated  & Deterministic & Power law \\
\hline
\texttt{s6} & Same as above & Same as above & Same as above & Same as above & Stochastic $I$/$Q$/$U$ \& spectral index & Power law \\
\hline
\texttt{s7} & Same as above & Same as above & Same +  $c_s$ from ARCADE & Same + fluctuations in $c_s$ & Deterministic & Curved power law \\
\enddata 
\end{deluxetable*}

\begin{deluxetable*}{m{0.5cm} m{4cm} m{4.4cm} m{2cm} m{3cm}} \label{table:summaryco}
...
\hline
\texttt{co3} & \centering Same as above & Simulated high galactic clouds & Deterministic & \centering Same as above  \\
\enddata
\end{deluxetable*}
...
\begin{table*}
   ...
   \bottomrule
    \end{tabular}
    \caption{Spectral indices adopted for synthesizing small scales, where $D_{\ell}^{XX} \propto \ell^{\alpha}$.}
