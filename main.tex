...
For the comparison of scaling in $\ell$, we define the ratio $\mathcal{R} \equiv \overline{\mathcal{D}_{\text{low-}\ell}^{BB}} \Big/ \overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$ as another metric to descirbe the change in spatial power across the modulation scale. Based on this definition, the fixed-index power law $\mathcal{D}_\ell^{BB} \propto \ell^{-0.42}$, derived from the analyses of larger sky regions ($f_\text{sky} = 0.3 \; \text{to} \; 0.8$) by \cite{planck2014-XXX}, yields a value of $\mathcal{R} = 1.60$. This estimate is closely aligned with the small-field NPIPE data, which yield a value of $\mathcal{R} = 1.85 \pm 0.93$. \texttt{d1}, \texttt{d9} and \texttt{d12} models instead give slightly larger values: $\mathcal{R} = 2.03 \pm 0.72$, $\mathcal{R} = 2.35 \pm 0.77$ and $\mathcal{R} = 2.26 \pm 0.91$ respectively. 
During the development of these models, particularly \texttt{d9}, we use this ratio as a proxy to monitor the transition of band power from large scales to small scales in the small-area cases, particularly for \texttt{d9}. This analysis guided us to adopt the new modulation map construction method discussed in Section~\ref{subsec:methodology}.
...
In Figure~\ref{fig:BKfield_power}, we show the \texttt{d9} and \texttt{d12} $BB$ band power at 353~GHz and the \texttt{s5} band power at 23~GHz, the pivot frequencies adopted in the BK analyses. These PySM power spectra are compared with the BK18 maximum likelihood foreground model comprising a modified blackbody power spectrum ($T_d$ fixed at $19.6~\text{K}$) with parameter $A_{d,\ell=80} = 4.4 \; \mu\rm{K^2_{CMB}}$, $\beta_d = 1.5$ and $\alpha_d = -0.66$ for dust, and a square-law power spectrum with parameter $A_{s,\ell=80} = 0.6 \; \mu\rm{K^2_{CMB}}$, $\beta_s = -3.0$ and $\alpha_s = 0.00$ for synchrotron. These BK18 model lines are further suppressed by their band power window functions to account for the mode loss and mode coupling induced by the matrices. All band powers are eventually converted to full-sky $D_\ell$ by a division of the integral of the band power window function.
