...
\begin{deluxetable*}{m{0.5cm} m{2.5cm} m{2.2cm} m{2.4cm} m{2.2cm} m{2.5cm} m{2.8cm}} \label{table:summarydust}
\caption{Summary of the PySM 3.4 models --- Dust. All models have a maximum $N_\text{side} = 8192$, except \texttt{d12} (2048).}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} & \colhead{Model} \\ & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{d9} & GNILC PR2 ($I$) + GNILC PR3 ($Q$/$U$) 353 GHz & Modulated + polarization tensor & \centering Uniform spectral parameters & \centering ---  & \centering Deterministic & Modified blackbody \\
\hline
\\
\texttt{d10} & \centering Same as above & Same as above &  \centering $\beta_d, \, T_d$ from GNILC & \centering Modulated & \centering Deterministic & \centering Same as above \tabularnewline
\hline
\texttt{d11} & \centering Same as above & Same as above & Same as above & \centering Same as above & Stochastic $I$/$Q$/$U$ \& spectral parameters & \centering Same as above \tabularnewline
\hline
\texttt{d12} &  GNILC PR2 ($I$) + PR3 ($Q$/$U$) 353 GHz & Modulated + gaussian & \centering Random realization for each layer & \centering  Random realization for each layer & \centering Deterministic & 6 layers, each modelled as a different modified blackbody \\
%\bottomrule
%\end{tabular}
%\end{center}
\enddata
...
\begin{deluxetable*}{m{0.5cm} m{2.5cm} m{2.2cm} m{2.4cm} m{2.2cm} m{2.5cm} m{2.8cm}} \label{table:summarysynch}
\caption{Summary of the PySM 3.4 models --- Synchrotron. All models have a maximum $N_\text{side} = 8192$.}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Templates} & \colhead{Templates} & \colhead{Frequency scaling} & \colhead{Frequency scaling} & \colhead{Stochasticity} & \colhead{Model} \\ & \colhead{at large scales} & \colhead{at small scales} & & \colhead{at small scales}}
\startdata
\texttt{s4} & Haslam ($I$) 408 MHz + WMAP ($Q$/$U$) 23 GHz & Modulated + polarization tensor & \centering Uniform spectral index & \centering --- & \centering Deterministic & \centering Power law \tabularnewline
\hline
\\
\texttt{s5} & \centering Same as above & Same as above & $\beta_s$ from Haslam, S-PASS, WMAP & \centering Modulated & \centering Deterministic & \centering Power law \tabularnewline
\hline
\texttt{s6} & \centering Same as above & Same as above & Same as above & \centering Same as above & Stochastic $I$/$Q$/$U$ \& spectral index & \centering Power law \tabularnewline
\hline
\\
\texttt{s7} & \centering Same as above & Same as above & Same +  $c_s$ from ARCADE & \centering Same + $c_s$ fluctuations & \centering Deterministic & \centering Curved power law \tabularnewline
\enddata 
\end{deluxetable*}

\begin{deluxetable*}{m{0.5cm} m{4.2cm} m{4.4cm} m{2cm} m{3cm}} \label{table:summaryco}
\caption{Summary of the PySM 3.4 models --- CO. All models have a maximum $N_\text{side} = 2048$.}
\tablewidth{0pt}
\tablehead{
\colhead{Tag} & \colhead{Templates} & \colhead{Templates} & \colhead{Stochasticity} & \colhead{Model} \\ & \colhead{at large scales} & \colhead{at small scales}}
\startdata
\texttt{co1} & \centering PR1 MILCA smoothed to $1^{\circ}$ & \centering --- & Deterministic & Single line emissions at 115, 230, 346~GHz \\
\hline
\texttt{co2} & \centering Same as above & \centering ---  & Deterministic & Same as above + $0.1\%$ polarized \\
\hline
\\
\texttt{co3} & \centering Same as above & Simulated high galactic clouds & Deterministic & \centering Same as above \tabularnewline
\enddata
\end{deluxetable*}
...
\begin{table*}
   ...
   \bottomrule
    \end{tabular}
    \caption{Model parameters adopted for synthesizing small scales, where $D_{\ell}^{XX} \propto \ell^{\alpha}$.}
...
Another challenge for our prescription is pixels that, likely from noise, have $I^2 < P^2$, rendering the transformation in Equation~\eqref{eq:real2pt} invalid. We identify 527 such pixels in our dust template and 5 in our synchrotron template, all in the vicinity of the Crab Nebula. Thus, before transforming the $I$, $Q$, and $U$ templates into $i$, $q$, and $u$, we replace these pixels with fiducial values of 4.51, 0.07, and 0.01\,$\mu$K$_{\rm RJ}$ in the dust $I$, $Q$, and $U$ maps and with 4.55, 0.017, $-0.001$\,$\mu$K$_{\rm RJ}$ in the synchrotron $I$, $Q$, and $U$ maps. Given these complications, we do not advise using the simulations in the vicinity of the Crab Nebula.
...
\begin{table}
   ...
   \bottomrule
    \end{tabular}
    \caption{Model parameters adopted for synthesizing small-scale dust and synchrotron emission, where $\mathcal{D}_\ell^{XX} \propto \ell^\alpha$.}
...
Table~\ref{tab:BB_dustratio} further explores how the model amplitudes vary across frequencies, presenting the deviation ratios down to 85~GHz. \texttt{d9} consistently maintains a deviation factor of around 2, while \texttt{d1} and \texttt{d10} deviate less from the measurements at lower frequencies, suggesting a preference for a slightly larger $\beta_d$ compared to the BK18 model's value 1.5. The irregular trend observed in \texttt{d12} reflects the decorrelation behavior inherent to this model. 
