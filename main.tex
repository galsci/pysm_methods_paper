...
To compare the performance of the latest PySM dust models \texttt{d9}, \texttt{d10} and \texttt{d12} with real data over a broad range of scales, we present their power spectra computed with both large-area and small-area masks at 353~GHz where the dust emissions dominate. We first generate PySM dust models maps at 353~GHz \sg{(\textbf{specify the resolution of the PySM maps})}, and color-correct the Planck NPIPE 353 GHz channel maps (i.e. scale them by a factor of 0.92) to the same single frequency. Additionally, we subtract the CMB dipole from the NPIPE temperature maps. We then apply identical masks (defined below) to the mean-subtracted PySM dust models and NPIPE ``A''/``B'' detector-split maps. We compute the cross-spectra of the masked NPIPE data splits to minimize noise correlation. We calculate the auto-spectra of the model maps with the masks.
...
We have purposefully chosen an earlier release of BeyondPlanck for our analysis. Later versions of BeyondPlanck, or its successor, CosmoGlobe produce synchrotron intensity maps at 408 MHz. The PySM synchrotron models s5 and s7 are not suitable for producing synchrotron simulations at 408 MHz. \sg{[\textbf{Move the technical reason for this to synchrotron model section and just refer to that reason.}]} This is a consequence of scaling the Haslam map from 408 GHz to 30 GHz for constructing the template for synchrotron intensity. When this template is rescaled to 408 MHz, with scaling that varies over the the sky, the model fails. We do not recommend use of the s5 and s7 models below 1 GHz. Therefore, we validate our models with the synchrotron intensity map at 30 GHz.
...
Applying the same process to other PySM models at frequencies of interest, we acquire a set of BK pipeline propagated, pure B-mode model foreground maps compatible with BK measurements.
% realistic B-mode measurements of the sky patch. 
We then proceed to calculate their $BB$ auto power spectra using the BK binning. In Figure~\ref{fig:BKfield_power}, we show the \texttt{d9} and \texttt{d12} $BB$ band power at 353~GHz and the \texttt{s5} band power at 23~GHz, the pivot frequencies adopted in the BK analyses. These PySM power spectra are compared with the BK18 maximum likelihood foreground model comprising a modified blackbody power spectrum ($T_d$ fixed at $19.6~\text{K}$) with parameter $A_{d,\ell=80} = 4.4 \; \mu\text{K}^2$ , $\beta_d = 1.5$ and $\alpha_d = -0.66$ for dust, and a square-law power spectrum with parameter $A_{s,\ell=80} = 0.6 \; \mu\text{K}^2$, $\beta_s = -3.0$ and $\alpha_s = 0.00$ for synchrotron. 
% These BK18 model lines are suppressed by its band power window function to account for the mode loss and mode coupling induced by the observation matrix, purification matrix, masking and binning as well. 
