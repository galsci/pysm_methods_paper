...
Both \texttt{d1} and \texttt{d9} show power amplitudes generally consistent with NPIPE, with \texttt{d9} demonstrating a significant improvement in the correlation, raising the correlation coefficient from 0.887 to 0.998 in the low-$\ell$ case. This enhancement is especially evident in the regimes of high signal-to-noise ratio, such as in the $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ comparison or in small fields with large dust amplitude. However, in fields where $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ or $\overline{\mathcal{D}_{\text{low-}\ell}^{BB}}$ are smaller than $10 \; \mu\rm{K_{CMB}}^2$, the \texttt{d9} model systematically overestimates the dust amplitude, which is a feature we also observe in a comprehensive analysis in the BICEP/Keck sky patch in Section~\ref{sec:BK_validation}. Conversely, the $\texttt{d12}$ model underestimates the dust amplitude in 94\% (85\%) of the small fields for low-$\ell$ (high-$\ell$). 

On the scaling-in-$\ell$, we define the ratio $\mathcal{R} \equiv \overline{\mathcal{D}_{\text{low-}\ell}^{BB}} \Big/ \overline{\mathcal{D}_{\text{high-}\ell}^{BB}}$ as another proxy for describing the spatial power change across the modulation scale. The perfect fixed-index power law $\mathcal{D}_\ell^{BB} \propto \ell^{-0.42}$ yields a value of $\mathcal{R} = 1.60$, while the NPIPE maps give $\mathcal{R} = 1.85 \pm 0.93$ and the \texttt{d1}, \texttt{d9} and \texttt{d12} model maps give $\mathcal{R} = 2.03 \pm 0.72$, $\mathcal{R} = 2.35 \pm 0.77$ and $\mathcal{R} = 2.26 \pm 0.91$ respectively over sky patches with $|b| > 30^\circ$.
