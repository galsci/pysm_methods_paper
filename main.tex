...
The $TT$, $EE$ and $BB$ power spectra comparisons for models \texttt{s5} \sg{and \texttt{s1}} are shown in Figure~\ref{fig:Dl_sync_galmask}. The comparison for models \texttt{s4}, \texttt{s6} and \texttt{s7} have nearly identical results, so we only show the results for model \texttt{s5}. \sg{The $TT$ power spectra for all sky fractions, for all models, show excellent agreement with the BeyondPlanck synchrotron temperature power spectra. For polarization $E$ and $B$ modes, model \texttt{s5} show good agreement in the multipole range where signal 
