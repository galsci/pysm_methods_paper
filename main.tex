...
We only show the PySM synchrotron \texttt{s5} and \texttt{s6} power spectra in the same figure since other new models yield approximately the same results in this sky patch. It is obvious that these models have a non-zero $\alpha_s$ and a several times larger amplitude at the pivot frequency. This picture however reverses at 40~GHz when the synchrotron amplitude from the BK constraint becomes several times larger instead, implying that the models have a more negative $\beta_s$ as well. We note that the latest BICEP/Keck results only imposes weak constraints on the synchrotron amplitude from the reprocessing WMAP 23/33~GHz and NPIPE 30/40 ~GHz maps. 

find that The discrepancies 
