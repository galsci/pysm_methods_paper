...
\end{equation}
\af{This transformation can be easily computed analytically by applying logarithm to eigenvalue decomposition of $P_{ab}$, and preserves polarization direction, but compresses Stokes parameters $I$, $Q$, $U$ into their polarization fraction tensor analogues $i$, $q$, $u$}
\begin{align}\label{eq:real2pt}
    ...
    ~~~,
\end{align}
where $P \equiv \sqrt{Q^2 + U^2}$ \af{is the usual polarization invariant}.
\af{For small polarization, these reduce to familiar quantities $i\simeq\ln I$, $q\simeq Q/I$, and $u\simeq U/I$, but for large polarization these variables have some nicer properties. In particular, inverse transformation}
\begin{align}\label{eq:pt2real}
    ...
    ~~~,
\end{align}
where $\xi \equiv \sqrt{q^2 + u^2}$, \af{guarantees positive intensity $I$, and will never result in polarization fraction $p > 1$ as $p=\tanh\xi$}. The transformation is non-linear, and thus Gaussian fluctuations introduced in $i$, $q$, and $u$ maps will necessarily become non-Gaussian when transformed back to $I$, $Q$, and $U$. \af{One has to be a bit careful when working with noisy maps (as noise is going to get entangled as well), but applications of this method to radio astronomy data yield significant improvements in reconstruction quality \citep{Arras:2021}.}
...
With the same process applied to other PySM models at frequencies of interest, we acquire a set of filtered and purified foreground polarization maps which represent the foreground outcomes from realistic B-mode measurements of the sky patch. We then proceed to calculate their $BB$ auto power spectra using the BK binning. In Figure~\ref{fig:BKfield_power}, we show the \texttt{d9} and \texttt{d12} $BB$ band power at 353~GHz and the \texttt{s5} and \texttt{s6} band power at 23~GHz, the pivot frequencies adopted in the BK analyses. These PySM power spectra are compared with the BK18 maximum likelihood foreground model comprising a modified blackbody power spectrum ($T_d$ fixed at $19.6~\text{K}$) with parameter $A_{d,\ell=80} = 4.4 \; \mu\text{K}^2$ , $\beta_d = 1.5$ and $\alpha_d = -0.66$ for dust, and a square-law power spectrum with parameter $A_{s,\ell=80} = 0.6 \; \mu\text{K}^2$, $\beta_s = -3.0$ and $\alpha_s = 0.00$ for synchrotron. These BK18 model lines are suppressed by its band power window function to account for the mode loss and mode coupling induced by the observation matrix, purification matrix, masking and binning as well. 
...
We present only the PySM \texttt{s5} and \texttt{s6} power spectra in the same figure as other new models yield similar results in this sky patch. These models clearly exhibit a non-zero $\alpha_s$ and a significantly larger amplitude at the pivot frequency. However, this trend reverses at 40~GHz, where the synchrotron amplitude from the BK data becomes several times larger instead, suggesting that the models have a more negative $\beta_s$. It is important not to overinterpret these discrepancies between our models and the data since the latest BK results provide weak constraints on the synchrotron foreground, relying on reprocessed WMAP 23/33~GHz and NPIPE 30/40~GHz maps. Revisiting the discrepancies will be interesting when new synchrotron measurements are released from the BICEP Array telescope which comprises a dedicated 30/40~GHz receiver~\citep{Moncelsi:2020}. 
...
Of note is that the current models do not explicitly impose any non-zero parity-odd correlations in the polarized dust emission, i.e., the $TB$ and $EB$ correlations are zero in the power spectra that are used to extrapolate the small-scale dust emission structure. However, analysis of the Planck polarized dust emission finds a significant nonzero $TB$ correlation at $\ell \lesssim 600$ \citep{}. A proposed physical mechanism for the origin of the nonzero $TB$ signal is misalignment between dusty filaments and the projected magnetic field orientation \citep{Huffenberger:2020, Clark:2021, Cukierman:2023}: this picture also predicts the sign and amplitude of an expected nonzero $EB$ correlation. Future work could incorporate this parity-odd contribution to the polarized dust emission. Such models would be of particular use for the development of analysis techniques that seek to constrain signatures of cosmic birefringence or other parity-violating physics in the CMB \citep[e.g.,][]{Minami:2020, Eskilt:2022}.
...
\begin{enumerate}
    \item We present new models of small-scale ..
