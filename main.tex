...
\begin{figure}
    ...
    \caption{Comparison of the \texttt{PySM} and NPIPE $BB$ power spectra for the BK patch against the BK18 foreground measurements. The top panel shows the dust results at 353~GHz, while the bottom panel presents the synchrotron results at 23~GHz. These nine bandpowers correspond to the BK science bins used for delivering constraints. For dust, the BK18 data points (offset horizontally for clarity) represent the most probable values with error bars indicating 68\% credible intervals. For synchrotron, 95\% upper limits are shown instead. Upper limits exceeding 5~$\mu\rm{K^2_{CMB}}$ are not displayed in this figure.}
