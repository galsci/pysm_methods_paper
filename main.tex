...
At large angular scales $\ell \lesssim 80$, the PySM models are largely influenced by the underlying GNILC template, whereas the BK18 constraints on dust $BB$ power come primarily from BK measurements at 220~GHz. The NPIPE power spectrum in the same plot reveals that the variations seen in the first three band powers of the \texttt{d9} model are directly inherited from its large-scale template. The remaining amplitude discrepancy is likely due to a positive noise bias present in the GNILC dust map. 
