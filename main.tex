...
\usepackage{amsmath}
\usepackage{booktabs} % BH: Compilation breaks without this, but I am not sure we need it if making standard aastex tables?
...
%since the PR3 data are contaminated by instrumental noise. 
...
In addition to the small-scale structure of the diffuse dust and synchrotron emission, the real sky at these frequencies contains emission from discrete sources that are not included in our models. These include cold clumps \citep{Clancy:2023}, planetary nebulae, supernova remnants, pulsar wind nebulae \citep{Guan:2021}, and so forth. 
...
\begin{enumerate}
    \item We develop new models of small-scale, non-Gaussian Galactic emission structure. We combine realizations of this small-scale synthetic emission with the latest component-separated Galactic emission data. We similarly extrapolate spectral parameters... The result is a set of all-sky models for   % BH: Call out polfrac
    \item Other models implemented... (MKD and CO)
    \item Improve agreement overall with data relative to previous PySM dust and synchrotron models. (We demonstrate that the power spectra of our models broadly agree with data from Planck over large sky areas.) In particular, Figure 6: improvement over previous PySM models in small patch BB power and ell-dependence
    \item CIB contamination
    \item We quantify the departure from non-Gaussianity of the small-scale structure introduced by the polarization fraction tensor framework. Probably something on non-Gaussianity: The polarization fraction tensor framework is 
    ...
    \item Model suite
    \item PySM3+files/data availability (or move to final paragraph)
